...
const ExpoBarcodeScanner: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    checkBarcodeScannerPermission();
  }, []);

  const checkBarcodeScannerPermission = async () => {
    let getPermission: PermissionResponse =
      await BarCodeScanner.getPermissionsAsync();
    let isGranted = getPermission.granted;
    if (!isGranted) {
      const reqPermission: PermissionResponse =
        await BarCodeScanner.requestPermissionsAsync();
      isGranted = reqPermission.granted;
    }
  ...
  if (!hasPermission) {
    return (
      <View style={styles.centeredView}>
        ...
        <Button
          title={'Request Again'}
          onPress={checkBarcodeScannerPermission}
        />
      </View>
    );
  }
        ...
        <>
          <BarCodeScanner
            ...
            style={[
              ...
              styles.container,
              {width: screenWidth * 1.2, height: screenHeight},
            ]}
          />
